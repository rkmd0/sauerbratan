...
namespace game
{
    ...
    void toserver(char *text) { conoutf(CON_CHAT, "%s:\f0 %s", chatcolorname(player1), text); addmsg(N_TEXT, "rcs", player1, text); append_to_file("bratan_chat.txt", text, chatcolorname(player1), nullptr); if(demorecord) recordmsg(N_TEXT, "rcs", player1, text); }
    ...
    void parsemessages(int cn, fpsent *d, ucharbuf &p)
    {
        ...
        while(p.remaining()) switch(type = getint(p))
        {
            ...
            case N_SWITCHNAME:
                getstring(text, p);
                if(d == player1 && renameprotection) {
                    filtertext(text, text, false, false, MAXNAMELEN);
                    if(!text[0]) copystring(text, "unnamed");
                    conoutf("\f3warning:\f7 attempt to rename you to \f3%s", colorname(d, text));
            ...
            case N_PONG:
                // wont work if server ignores N_CLIENTPING
                /*if(hideping) {
            ...
            if(demorecord) 
                recordmsg(N_CLIENTPING, "i", player1->ping);
